...
BBox::BBox(REAL_TYPE _minx, REAL_TYPE _miny, REAL_TYPE _minz, REAL_TYPE _maxx, REAL_TYPE _maxy, REAL_TYPE _maxz) {
	min[0] = _minx; min[1] = _miny; min[2] = _minz;
	max[0] = _maxx; max[1] = _maxy; max[2] = _maxz;
}

BBox::BBox(REAL_TYPE _min[3], REAL_TYPE _max[3]) {
	min[0] = _min[0]; min[1] = _min[1]; min[2] = _min[2];
	max[0] = _max[0]; max[1] = _max[1]; max[2] = _max[2];
}

BBox::BBox(const Vec3<REAL_TYPE>& _min, const Vec3<REAL_TYPE>& _max) {
	min = _min;
	max = _max;
}
...
void BBox::setMinMax(const Vec3<REAL_TYPE>& _min, const Vec3<REAL_TYPE>& _max) {
	min = _min;
	max = _max;
}
...
Vec3<REAL_TYPE> BBox::getPoint(int idx) const 
{
	int x, y, z;
	Vec3<REAL_TYPE> p = min;
	x = idx    & 0x1;
	y = idx>>1 & 0x1;
	z = idx>>2 & 0x1;
	if (x) p[0] = max[0];
...
Vec3<REAL_TYPE> BBox::center() const 
{ 
...
REAL_TYPE BBox::xsize() const { return max[0] - min[0]; }
REAL_TYPE BBox::ysize() const { return max[1] - min[1]; }
REAL_TYPE BBox::zsize() const { return max[2] - min[2]; }
REAL_TYPE BBox::length(const AxisEnum& axis) const { return max[axis] - min[axis]; }
REAL_TYPE BBox::diameter() const { return (max - min).length(); }
AxisEnum BBox::getMaxAxis(REAL_TYPE& length) const {
	Vec3<REAL_TYPE> len = size();
	AxisEnum axis;
	if (len[0] > len[1]) {
...
BBox BBox::getCrossedRegion(BBox& other_bbox) const {
	...
	// 点の包含判定
	std::list<Vec3<REAL_TYPE> > inc_own_points;
	std::list<Vec3<REAL_TYPE> > inc_other_points;
	for (int i=0; i<8; i++) {
		// 自分に相手の8頂点が含まれるかチェック
	...
	std::list<Vec3<REAL_TYPE> > cross_own_face_with_other_side;
	for (int axis_id = 0; axis_id < 3; axis_id++) {
		Vec3<REAL_TYPE> face[2][2];	//face[2面][長方形の対角点]
		Vec3<REAL_TYPE> side[4][2];	//side[4辺][辺の両端の2点] :対点はmin-maxの順
		...
		for (int face_id = 0; face_id < 2; face_id++) {
			for (int side_id = 0; side_id < 4; side_id++) {
				...
				REAL_TYPE z0 = (side[side_id][0])[axis_id];//辺id,点id,xyz
				REAL_TYPE z1 = (side[side_id][1])[axis_id];//辺id,点id,xyz
				REAL_TYPE z  = (face[face_id][0])[axis_id];//面id,点id,xyz

				if (z0 <= z && z <= z1) {
					// 2次元平面と点との包含判定
					Vec2<REAL_TYPE> min = vec3to2(axis_id,face[face_id][0]);
					Vec2<REAL_TYPE> max = vec3to2(axis_id,face[face_id][1]);
					Vec2<REAL_TYPE> pos = vec3to2(axis_id,side[side_id][0]);
	...
	std::list<Vec3<REAL_TYPE> > cross_other_face_with_own_side;
	for (int axis_id = 0; axis_id < 3; axis_id++) {
		Vec3<REAL_TYPE> face[2][2];	//face[2面][長方形の対角点]
		Vec3<REAL_TYPE> side[4][2];	//side[4辺][辺の両端の2点] :対点はmin-maxの順
	...
	BBox cross_bbox;
	cross_bbox.init();
	std::list<Vec3<REAL_TYPE> >::iterator itr;
...
///
Vec2<REAL_TYPE> BBox::vec3to2(int axis_id, Vec3<REAL_TYPE>& v3) const {
