...
// #endif
	 ...
	 for( j=0; j<num_trias; j++ ) {
	   ...
	   vertex_ptr[0] = new Vertex<T>(p_triaarray[pos_tria],
					 p_triaarray[pos_tria+1],
					 p_triaarray[pos_tria+2]);
	   vertex_ptr[1] =  new Vertex<T>(p_triaarray[pos_tria+3],
					  p_triaarray[pos_tria+4],
					  p_triaarray[pos_tria+5]);
	   vertex_ptr[2] =  new Vertex<T>(p_triaarray[pos_tria+6],
	   ...
	   tria_vec.push_back( new PrivateTriangle<T>(vertex_ptr,
						      p_idarray[pos_id]));
	   //	   tria_vec.push_back(
...
)
{
	...
	PrivateTriangle<T>* p_tria=p_trias->at(0);
	DVertexTriangle<T>* p_dvtria=NULL;
	try{
	  p_dvtria=dynamic_cast<DVertexTriangle<T>*>(p_tria);
	}
	catch ( std::bad_cast ){
	  std::cerr << "bad cast"<<std::endl;
	}
	
	DVertex<T>** tmp_dvlist= p_dvtria->get_DVertex();
	DVertex<T>* dvtmp= tmp_dvlist[0];
...
template <typename T>
POLYLIB_STAT
MPIPolylib<T>::pack_tria_scalar_data(
	std::vector<T>* p_vec,
	const std::vector<PrivateTriangle<T>*>* p_trias
)
{
	...
	for( unsigned int i=0; i<p_trias->size(); i++ ) {
	  ...
	  PrivateTriangle<T>* p_tria=p_trias->at(i);
	  DVertexTriangle<T>* p_dvtria=NULL;
	  try{
	    p_dvtria=dynamic_cast<DVertexTriangle<T>*>(p_tria);
	  }
...
template <typename T>
POLYLIB_STAT
MPIPolylib<T>::pack_tria_vector_data(
	std::vector<T>* p_vec,
	const std::vector<PrivateTriangle<T>*>* p_trias
)
{
...
template <typename T>
POLYLIB_STAT
MPIPolylib<T>::gather_polygons_vtk(){
  ...
  //#define DEBUG
	...
	PL_DBGOSH << "MPIPolylib::gather_polygons_vtk() MPITAG_TRIA_NDATA. calc" << std::endl;
		...
		T *p_tria_scalar_array = new T[ total_tria_nscalar ];
	...
	PL_DBGOSH << "MPIPolylib::gather_polygons_vtk() MPITAG_TRIA_SCALAR. " << std::endl;
		...
		T *p_tria_vector_array = new T[ total_tria_nvector ];
...
template <typename T>
POLYLIB_STAT
MPIPolylib<T>::send_polygons_to_rank0_vtk(){
  //#define DEBUG
#ifdef DEBUG
	...
	PolygonGroup<T> *p_pg;
	std::vector<PrivateTriangle<T>*> const *p_trias;
	//std::vector<DVertexTriangle<T>*> const *p_dvtrias;
	...
	std::vector<T> send_tria_scalar;
	T        *p_send_tria_scalar_array;
	std::vector<T> send_tria_vector;
	T        *p_send_tria_vector_array;
	...
	p_send_tria_scalar_array = new T[send_tria_scalar.size()];
	for( i=0; i<send_tria_scalar.size(); i++ )
	  p_send_tria_scalar_array[i] = send_tria_scalar[i];

	p_send_tria_vector_array = new T[send_tria_vector.size()];
